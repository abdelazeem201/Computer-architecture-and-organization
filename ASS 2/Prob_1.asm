#
# Write a complete MIPS-32 program to collect an array of eight integers (in ascending order) by the user, and then perform Binary Search to find whether a value entered by the user exists in the array. Collect the numbers from the input console using a loop inside a function form array, and store in memory in an array called \array". Do not store the numbers as scalars in eight different non-contiguous locations or in eight different registers. After the input numbers are collected from the user, there should be sanity checking to ensure that the numbers in the array are actually in ascending order using a function check ascending array; if they are not, print an error message and exit. Otherwise, perform binary search by calling the function bin search, which always returns its result in register $v0. If the search is successful, the array index at which the element was found is returned, and if the search is unsuccessful, the function returns -1. Depending on the returned value, from inside main print the result to the user with a proper message. Follow all usual register usage conventions for recursive and non-recursive MIPS-32 function calls.
#
